...
class XArmBridge:
    ...
    def run(self):
        ...
        try:
            while True:
                # Flush ZMQ buffer to get the absolute newest command
                try:
                    while True:
                ...
                if self._latest_q is not None:
                    if (time.monotonic() - self._latest_rx_mono) <= self.timeout_s:
                        if self.use_gripper:
                            # Split arm joints and gripper command out cleanly
...
if __name__ == "__main__":
    ...
    parser.add_argument('robot_ip', type=str, nargs='?', default="192.168.42.222")
